...
```sh
GiGiOS/bin/shell-local.sh            # crea los que falten
GiGiOS/bin/shell-local.sh --check    # solo informa
GiGiOS/bin/shell-local.sh --force    # respalda y reemplaza los que no cargan el compartido
```
...
```sh
GiGiOS/bin/shell-local.sh   # recrea los locales
