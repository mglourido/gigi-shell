...
# `sistema`). NO se symlinkea desde ~/GiGiShell: corre como root vía /etc/sudoers.d/gigios-camara, y
...
case "${1:-}" in
    block)
        ...
        cat > "$REGLA" <<'EOF'
# Generado por gigios-camara (GiGiShell). Su PRESENCIA es el interruptor de bloqueo
