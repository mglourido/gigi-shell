...
| 8 | `gigios/compactar.lua` | `GiGiShell.compactar()` — renumera escritorios | a mano |
| 9 | `gigios/boton-apagado.lua` | `GiGiShell.boton_apagado()` — el botón físico | a mano |
| 10 | `gigios/tapa.lua` | `GiGiShell.tapa_cerrada()` / `GiGiShell.tapa_abierta()` — qué hace el portátil al cerrar la tapa (reutiliza las acciones de `boton-apagado`) | a mano — **la acción** la escribe AGS en `preferences.json` |
| 11 | `gigios/daltonismo.lua` | `GiGiShell.daltonismo(modo)` — shader de accesibilidad | a mano |
| 12 | `gigios/orion.lua` | `GiGiShell.toggle_orion()` — el atajo del launcher | a mano |
| 13 | `gigios/ancla-escritorio.lua` | `GiGiShell.anclar_escritorio()` / `GiGiShell.saltar_ancla()` — ir al escritorio ancla y volver | a mano |
| 14 | `gigios/keybinds.lua` | Todos los atajos + `GiGiShell.toggle_gaps()` | a mano |
