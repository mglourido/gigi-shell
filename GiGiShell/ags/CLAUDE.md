...
`out.css` **no se versiona**: es una caché. El autostart (`gigios/autostart.lua`) llama a ese
...
--source-map-urls=absolute`, mapa movido a `~/.cache/gigios/out.css.map`).
...
- `servicios/fondos/acento.ts` + `acentoCss.ts` + `acentoCache.ts` + `scripts/acento-fondo.py` —
  **el acento adaptativo**: los tres colores de acento del tema salen del fondo de escritorio. El
  ...
  - **El decodificado usa `im.draft()` antes del `convert`**: le pide al decodificador JPEG que
    ...
    que se miden.
- `servicios/energia/botonEncendido.ts` — **qué hace el botón de encendido físico** (Ajustes > Energía). El shell **solo persiste la elección** (`botonApagado` en `preferences.json`); quien la ejecuta es `GiGiShell.boton_apagado()` (`hypr/gigios/boton-apagado.lua`) desde el bind de `XF86PowerOff` con `{locked = true}`, releyéndola en cada pulsación — así el botón responde con AGS caído y con la sesión bloqueada, y el setter no tiene que relanzar ni recargar nada. La única acción que vuelve al shell es `menu`, por el `request` **`toggle-power-menu`** (`alternarMenuEnergia()`), añadido con ella.
