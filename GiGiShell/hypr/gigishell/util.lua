...
local json = require("gigishell.json")
...
---
--- `opts.crudo` quita el prefijo "[GiGiShell Lua]". El prefijo existe para que un
...
--- (gigishell/ancla-escritorio.lua). Para avisos de fallo, sin tocar.
function M.notificar(texto, opts)
  opts = opts or {}
  pcall(function()
    hl.notification.create({
      text = (opts.crudo and "" or "[GiGiShell Lua] ") .. texto,
...
--- ~/.config/gigishell/preferences.json, cacheado durante esta ejecución del config
...
function M.prefs()
  if _prefs_cache == nil then
    _prefs_cache = M.leer_json(M.HOGAR .. "/.config/gigishell/preferences.json") or {}
  end
  return _prefs_cache
end
...
-- aplican gigishell/pantalla.lua y gigishell/dispositivos.lua. Ver CLAUDE.md.
