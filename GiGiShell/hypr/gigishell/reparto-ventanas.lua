-- gigishell/reparto-ventanas.lua — que una ventana NUEVA no nazca estrujada.
--
...
-- `smart_split` (partir por donde tengas el cursor, ver gigishell/ventanas.lua).
...
--      gigishell/keybinds.lua, donde se usa para el mismo fin en
...
--     gigishell/limite-ventanas.lua, que corre después y puede mandarla a otro
--     escritorio; allí la coloca dwindle por su cuenta (el `preselect` de aquí
--     ya se consumió en el escritorio de origen).
--
-- Ajustes en ~/.config/gigishell/preferences.json, como maxVentanasEscritorio:
...
--- una address. Mismo recuento que gigishell/limite-ventanas.lua y que
...
-- (`bindm` de gigishell/keybinds.lua). Al soltar, dwindle reinserta la ventana
