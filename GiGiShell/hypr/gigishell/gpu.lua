...
--
-- Perfiles válidos = los módulos de gigishell/gpu/. Sin fichero o con un nombre
...
local ruta = util.HOGAR .. "/.config/gigishell/gpu-perfil"
local nombre = (util.leer_fichero(ruta) or ""):gsub("%s+", "")

if nombre == "" then
  util.notificar("sin perfil de GPU: escribe uno en ~/.config/gigishell/gpu-perfil "
...
elseif not VALIDOS[nombre] then
  ...
  util.carga("gigishell.gpu." .. nombre)
end
