...
-- gigishell/variables.lua (lo que en hyprlang eran las $variables).
--
-- ═══════════════════════════════════════════════════════════════════════════
...
-- es la que gigishell/nop-binds.lua consulta para NO poner un bind sordo encima
...
local vars = require("gigishell.variables")

local mod = vars.mainMod

-- gigishell/ventanas.lua presta el envoltorio que apaga dwindle:smart_split
...
-- propósito: GiGiShell.compactar la define gigishell/compactar.lua y el orden de
...
-- modo NO se autoarranca (no hay línea suya en gigishell/autostart.lua) — enciende
...
-- soltado un ratón). Ver gigishell/ventanas.lua.
for i = 1, 10 do
...
-- El motor está en gigishell/ancla-escritorio.lua. Aquí vivía el workspace especial `magic` (toggle_special + mover ventana al
...
-- Dos binds MÁS sobre la misma combinación que el arrastre: gigishell/reparto-ventanas.lua
...
-- GiGiShell.boton_apagado (gigishell/boton-apagado.lua — enlace tardío, este módulo
...
-- CIERRE por GiGiShell.tapa_cerrada (gigishell/tapa.lua — enlace tardío, este módulo
-- NO lo requiere: lo carga el entry point).
...
-- Abrir la tapa enciende la pantalla SIEMPRE (ver el porqué en gigishell/tapa.lua):
...
-- Orion. Enlace TARDÍO como los demás GiGiShell.*: lo define gigishell/orion.lua, que
...
-- de gigishell/ventanas.lua, que es quien los aplica. Antes eran literales
...
-- `usados`/`normalizar` los consume gigishell/nop-binds.lua (require cachea: es
